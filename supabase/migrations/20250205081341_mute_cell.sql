...
DROP FUNCTION IF EXISTS check_user_role(uuid, user_role_enum);
...
-- Create role management functions with unique names
CREATE OR REPLACE FUNCTION verify_user_role(user_id uuid, required_role user_role_enum)
RETURNS boolean
LANGUAGE sql
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION verify_admin_status(user_id uuid)
RETURNS boolean
LANGUAGE sql
SECURITY DEFINER
SET search_path = public
